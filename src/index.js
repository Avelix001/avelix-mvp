...
async function verificationCostForType(env, credentialType){
  try {
    const row=await env.DB.prepare("SELECT value FROM platform_settings WHERE key='verification_prices_json'").first();
    const map=JSON.parse(row?.value||'{}');
    const key=String(credentialType||'').trim().toLowerCase();
    const n=Math.floor(Number(map[key]));
    if(Number.isFinite(n) && n>0) return Math.max(1,Math.min(100000,n));
  } catch(e) {}
  try { const row=await env.DB.prepare("SELECT value FROM platform_settings WHERE key='verification_cost_avx'").first(); const n=Math.floor(Number(row?.value||RINGO_DEFAULT_COST)); return Math.max(1,Math.min(100000,n)); } catch(e){ return RINGO_DEFAULT_COST; }
}
async function verificationCost(env){ return verificationCostForType(env,''); }
async function adminVerificationPrices(request, env){
  const admin=await requireAdmin(request,env); if(!admin) return adminDenied();
  const b=await request.json().catch(()=>({})); const input=b.prices||{}; const out={};
  for(const type of RINGO_ALLOWED_CREDENTIAL_TYPES){ const key=String(type).toLowerCase(); const n=Math.floor(Number(input[key])); if(Number.isFinite(n)&&n>0&&n<=100000) out[key]=n; }
  await env.DB.prepare(`INSERT INTO platform_settings(key,value,updated_at) VALUES('verification_prices_json',?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP`).bind(JSON.stringify(out)).run();
  await env.DB.prepare('INSERT INTO audit_logs(actor_user_id,action,target_type,target_id,details) VALUES(?,?,?,?,?)').bind(admin.user_id,'verification.prices.update','setting','verification_prices_json',JSON.stringify(out)).run();
...
async function submitCredentialVerification(request, env){
  ...
  const cost=await verificationCostForType(env, credentialType);
  ...
  const debit=await env.DB.prepare(`UPDATE profiles SET avx_balance=avx_balance-? WHERE id=? AND avx_balance>=?`).bind(cost,user.id,cost).run();
  if(Number(debit.meta?.changes||0)!==1) return json({error:`You need ${cost} AVX for this verification.`},402);
  let result;
  try {
    ...
    await env.DB.prepare(`INSERT INTO avx_transactions(profile_id,type,amount,reference,note) VALUES(?,?,?,?,?)`).bind(user.id,'verification_charge',-cost,ref,`Credential verification at ${institution.name}`).run();
    await env.DB.prepare(`INSERT INTO audit_logs(actor_user_id,action,target_type,target_id,details) VALUES(?,?,?,?,?)`).bind(user.user_id,'verification.submit','verification_submission',String(result.meta?.last_row_id||''),JSON.stringify({institution_id:institutionId,institution:institution.name,cost,reference:ref})).run();
  } catch(e) {
    await env.DB.prepare('UPDATE profiles SET avx_balance=avx_balance+? WHERE id=?').bind(cost,user.id).run();
...
function pageAdSlider(title='AVELYX') {
  ...
  const key=String(title).toLowerCase(); let order=[0,1,2,3,4];
...
async function dashboardPage(request, env){const serverUser=await currentUser(request,env);if(!serverUser)return Response.redirect(new URL('/login.html',request.url),302);return shell('Dashboard',`<section class="dash-ad-banner"><div><span class="eyebrow">AVELYX • YOUR WORKSPACE</span><h1 id="dashTitle">Your professional identity, organized.</h1><p id="dashSubtitle">Manage your profile, verification, secure sharing and AVX services from one clean workspace.</p></div><span class="dash-type-chip" id="dashType">MEMBER</span></section><section class="dash-focus" id="dashFocus"><div><span class="focus-icon" id="focusIcon">◉</span><div><span class="eyebrow" id="focusEyebrow">IDENTITY</span><h2 id="focusTitle">Your AVELYX Profile</h2><p id="focusText">Keep your professional identity complete and ready to share.</p></div></div><a class="btn" id="focusAction" href="/profile.html">Open Profile</a></section><section class="quick-grid" id="dashQuick"></section><section class="dash-panels"><div class="status-panel"><div class="eyebrow">IDENTITY STATUS</div><div id="identityBox"><b>AVELYX ID</b><br><span class="muted">Your workspace is ready.</span></div><a class="text-link" href="/profile.html">Manage profile →</a></div><div class="status-panel"><div class="eyebrow">AVX BALANCE</div><div id="walletBox"><b>AVX Wallet</b><br><span class="muted">Open wallet to view balance.</span></div><a class="text-link" href="/wallet.html">Open wallet →</a></div></section><section class="dash-promo"><div><div class="eyebrow" id="promoEyebrow">AVELYX</div><h2 id="promoTitle">Prove Your Potential.</h2><p id="promoText">Build it. Verify it. Share it.</p></div><a href="/verification.html" class="btn ghost" id="promoAction">Verify a Credential</a></section>`,`<style>.dash-ad-banner{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-top:4px;padding:20px 22px;border:1px solid #252f50;border-radius:20px;background:linear-gradient(100deg,#0b1128,#0a0f20 62%,#11152b)}.dash-ad-banner h1{font-size:clamp(26px,4.5vw,42px);margin:7px 0 5px}.dash-ad-banner p{margin:0;color:#8e9cbe;line-height:1.55;max-width:720px}.dash-type-chip{padding:8px 11px;border-radius:999px;border:1px solid #3a4262;background:#11182e;color:#bfa8ff;font-size:9px;font-weight:950;letter-spacing:.14em;white-space:nowrap}.dash-focus{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-top:12px;padding:18px 20px;border-radius:20px;border:1px solid #252f50;background:linear-gradient(145deg,#0d1530,#080d20)}.dash-focus>div{display:flex;align-items:center;gap:14px}.focus-icon{width:48px;height:48px;display:grid;place-items:center;border-radius:15px;background:linear-gradient(145deg,#6e48d6,#1b2345);color:#fff;font-size:21px;font-weight:950}.dash-focus h2{margin:4px 0;font-size:20px}.dash-focus p{margin:0;color:#8997b8;font-size:12px}.quick-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:11px;margin-top:12px}.quick-card{position:relative;display:flex;align-items:center;gap:12px;min-height:88px;padding:14px;border-radius:17px;border:1px solid #202c50;background:linear-gradient(145deg,#0d1530,#080d20);color:#fff;text-decoration:none;transition:.18s}.quick-card:hover{transform:translateY(-2px);border-color:#7350c9}.quick-icon{width:40px;height:40px;flex:0 0 40px;display:grid;place-items:center;border-radius:13px;background:#121b38;border:1px solid #303d61;color:#c7b5ff;font-size:18px;font-weight:900}.quick-card b{display:block;font-size:13px;margin-bottom:4px}.quick-card small{display:block;color:#8593b7;line-height:1.35}.quick-card em{position:absolute;right:8px;top:8px;font-style:normal;font-size:7px;letter-spacing:.12em;color:#7380a2}.dash-panels{display:grid;grid-template-columns:1.25fr .75fr;gap:11px;margin-top:12px}.status-panel{padding:18px;border:1px solid #202c50;border-radius:19px;background:linear-gradient(145deg,#0b1229,#080d20)}.status-panel #identityBox,.status-panel #walletBox{margin:9px 0 11px;min-height:48px}.status-panel #identityBox b{font-size:20px}.status-panel #walletBox b{font-size:28px}.text-link{color:#b99cff;text-decoration:none;font-size:11px;font-weight:800}.dash-promo{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-top:12px;padding:19px;border-radius:19px;border:1px solid rgba(212,174,74,.2);background:linear-gradient(105deg,rgba(139,77,255,.1),rgba(212,174,74,.055))}.dash-promo h2{margin:5px 0}.dash-promo p{margin:0;color:#9da9ca;font-size:12px}@media(max-width:900px){.quick-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.dash-ad-banner{align-items:flex-start}.dash-type-chip{display:none}.dash-focus{display:block}.dash-focus .btn{display:inline-block;margin-top:14px}.quick-grid{grid-template-columns:1fr}.dash-panels{grid-template-columns:1fr}.dash-promo{display:block}.dash-promo .btn{display:inline-block;margin-top:13px}}</style>`+memberGuardScript()+`<script>const DASH_UX={individual:{label:'INDIVIDUAL',title:'Your professional identity, organized.',subtitle:'Manage your profile, credentials, secure sharing and opportunities from one private workspace.',focusEyebrow:'PERSONAL IDENTITY',focusTitle:'Your Professional Profile',focusText:'Keep your AVELYX identity complete, credible and ready to share.',focusIcon:'◉',focusAction:'Open Profile',promo:'Build your professional identity one verified step at a time.',cards:[['◉','My Profile','Identity, skills and readiness','/profile.html'],['✓','Verification','Verify a credential','/verification.html'],['▦','Secure QR','Share your identity','/qr.html'],['◆','Jobs','Find opportunities','/opportunities.html'],['⌘','Permissions','Control shared information','/permissions.html'],['◈','AVX Wallet','Balance and activity','/wallet.html'],['♢','Notifications','Requests and updates','/notifications.html'],['⌕','Search Network','Business • Individual • Global','#soon']]},entrepreneur:{label:'ENTREPRENEUR',title:'Your professional and venture hub.',subtitle:'Bring your personal identity, projects, services and opportunities into one premium workspace.',focusEyebrow:'PROFESSIONAL + VENTURE',focusTitle:'Your Entrepreneur Profile',focusText:'Present who you are, what you build and what you offer.',focusIcon:'✦',focusAction:'Open Profile',promo:'Build. Verify. Present your work with confidence.',cards:[['◉','Professional Profile','Identity and expertise','/profile.html'],['✦','Verification','Verify your credentials','/verification.html'],['◆','Opportunities','Jobs and growth opportunities','/opportunities.html'],['▦','Secure QR','Share your professional identity','/qr.html'],['⌘','Permissions','Control shared information','/permissions.html'],['◈','AVX Wallet','Balance and activity','/wallet.html'],['♢','Notifications','Requests and updates','/notifications.html'],['⌕','Search Network','Business • Individual • Global','#soon']]},business:{label:'BUSINESS',title:'Your company workspace.',subtitle:'Manage your company identity, recruitment verification, opportunities and secure business sharing.',focusEyebrow:'COMPANY IDENTITY',focusTitle:'Your Business Profile',focusText:'Keep your company information ready for clients, partners and verified personnel workflows.',focusIcon:'▣',focusAction:'Open Company Profile',promo:'Build a company identity people can verify and trust.',cards:[['▣','Company Profile','Company details and identity','/profile.html'],['✓','Credential Verification','Verify personnel credentials','/verification.html'],['◆','Recruitment & Jobs','Manage opportunities','/opportunities.html'],['▦','Company QR','Share your company identity','/qr.html'],['⌘','Permissions','Control shared information','/permissions.html'],['◈','AVX Wallet','Service credits and activity','/wallet.html'],['♢','Notifications','Requests and updates','/notifications.html'],['⌕','Search Network','Business • Individual • Global','#soon']]}};(async()=>{const p=await me();if(!p)return;const type=['business','entrepreneur'].includes(p.account_type)?p.account_type:'individual';const u=DASH_UX[type];document.body.dataset.accountType=type;document.getElementById('dashTitle').textContent=u.title;document.getElementById('dashSubtitle').textContent=u.subtitle;document.getElementById('dashType').textContent=u.label;document.getElementById('focusIcon').textContent=u.focusIcon;document.getElementById('focusEyebrow').textContent=u.focusEyebrow;document.getElementById('focusTitle').textContent=u.focusTitle;document.getElementById('focusText').textContent=u.focusText;document.getElementById('focusAction').textContent=u.focusAction;document.getElementById('promoText').textContent=u.promo;const q=document.getElementById('dashQuick');q.innerHTML=u.cards.map(([icon,title,sub,href])=>href==='#soon'?'<a class="quick-card" href="#" onclick="event.preventDefault();alert(\'AVELYX Search Network is coming soon.\')"><span class="quick-icon">'+icon+'</span><div><b>'+title+'</b><small>'+sub+'</small></div><em>SOON</em></a>':'<a class="quick-card" href="'+href+'"><span class="quick-icon">'+icon+'</span><div><b>'+title+'</b><small>'+sub+'</small></div></a>').join('');document.getElementById('identityBox').innerHTML='<b>'+esc(p.avx_id)+'</b><br><span class="pill">'+esc(u.label)+'</span> · '+esc((p.card_tier||'basic').toUpperCase())+'<br><small class="muted">'+esc(p.title|| (type==='business'?'Company workspace':'Professional identity'))+'</small>';document.getElementById('walletBox').innerHTML='<b>'+Number(p.avx_balance||0).toLocaleString()+' AVX</b><br><small class="muted">Service credits available</small>';try{const r=await fetch('/api/information-requests',{credentials:'same-origin'});const d=await r.json();const n=(d.requests||[]).filter(x=>x.status==='pending').length;if(n){const badge=document.getElementById('notificationBadge');if(badge){badge.style.display='block';badge.textContent=n}}}catch(e){console.warn('AVELYX dashboard notifications',e)}})()</script>`)}
...
async function platformConfig(request, env) {
  ...
  let verification_prices={}; try{verification_prices=JSON.parse(settings.verification_prices_json||'{}')}catch(e){} return json({settings,verification_prices,packages: packages.results || []});
}
async function adminPlatform(request, env) {
  ...
  let verification_prices={}; try{verification_prices=JSON.parse(settings.verification_prices_json||'{}')}catch(e){} return json({settings,verification_prices,packages:packages.results||[],profiles:profiles.results||[],transactions:transactions.results||[],jobs:jobs.results||[],treasury:treasury||null});
}
...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname === '/api/admin/verification-prices' && request.method === 'POST') return await adminVerificationPrices(request, env);
