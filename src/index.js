...
async function login(request, env) {
  try {
    ...
    if (!user || !user.password_hash || !(await verifyPassword(password, user.password_hash))) return json({ error: 'Invalid email or password.' }, 401);
    if (Number(user.email_verified) !== 1) return json({ error: 'Please verify your email before logging in.', needs_verification: true }, 403);
    const token = await createSession(user.id, env);
    return withSession(json({ ok: true }), token);
  } catch (e) {
    return json({ error: 'Login failed. Please try again.', detail: e?.message || String(e) }, 500);
  }
}
...
async function getDigitalCvData(env, profileId){
  await ensureDigitalCvTable(env);
  return await env.DB.prepare('SELECT * FROM digital_cv_data WHERE profile_id=?').bind(profileId).first() || {};
}
function cvList(value){
  return String(value||'').split(/\r?\n|\s*[,;]\s*/).map(x=>x.trim()).filter(Boolean);
}
function cvJson(value, fallback=[]){
  const raw=String(value||'').trim();
  if(!raw) return fallback;
  try { const x=JSON.parse(raw); return Array.isArray(x)?x:fallback; } catch(e) {}
  return raw.split(/\r?\n/).map(line=>line.trim()).filter(Boolean).map(line=>{ const a=line.split('\|').map(x=>x.trim()); return {degree:a[0]||'',field:a[1]||'',issuer:a[2]||'',role:a[1]||'',dates:a[2]||'',description:a.slice(3).join(' | '),company:a[0]||'',title:a[0]||'',award:a[0]||'',name:a[0]||''}; });
}
function maskEmail(value){ const v=String(value||''); const at=v.indexOf('@'); if(at<=1) return v ? '••••••@'+(v.split('@')[1]||'') : '••••••'; return v[0]+'••••••••'+v.slice(Math.max(1,at-2),at)+'@'+v.slice(at+1); }
function maskPhone(value){ const v=String(value||'').replace(/\s+/g,''); if(v.length<7) return v?'••••••':'••••••'; return v.slice(0,4)+' •••••• '+v.slice(-3); }
...
async function publicProfilePhoto(request, env, avxId){
  const p=await env.DB.prepare('SELECT id FROM profiles WHERE avx_id=?').bind(avxId).first();
  if(!p) return new Response('Not found',{status:404});
  const row=await env.DB.prepare('SELECT mime_type,data_base64 FROM profile_photos WHERE profile_id=?').bind(p.id).first();
  if(!row) return new Response('Not found',{status:404});
  try { const bytes=Uint8Array.from(atob(row.data_base64),c=>c.charCodeAt(0)); return new Response(bytes,{headers:{'content-type':row.mime_type,'cache-control':'public, max-age=300'}}); } catch(e){ return new Response('Invalid photo',{status:500}); }
}
...
async function scanShare(token, env, request) {
  const hash = await sha256(token);
  const ts = now();
  ...
  return renderDigitalCv(profile, env, {secure:true, publicUrl:new URL(`/s/${token}`,request.url).toString()});
}


const PERMISSION_FIELDS = ['full_name','title','organization','industry','location','website','bio','email','phone','skills','qualifications','certifications','cac_number'];
const PERMISSION_LABELS = {full_name:'Full name',title:'Professional title',organization:'Organization',industry:'Industry',location:'Location',website:'Website',bio:'About',email:'Email',phone:'Phone',skills:'Skills',qualifications:'Qualifications',certifications:'Certifications',cac_number:'CAC / RC number'};
...
async function renderDigitalCv(profile, env, opts={}) {
  const cv=await getDigitalCvData(env, profile.id);
  const credentials=(await env.DB.prepare("SELECT credential_type,title,issuer,reference,status,verified_at,expires_at,notes,created_at FROM credentials WHERE profile_id=? ORDER BY created_at DESC").bind(profile.id).all()).results||[];
  const verified=credentials.filter(c=>String(c.status||'').toLowerCase()==='verified');
  const soft=cvList(cv.soft_skills || '');
  const hard=cvList(cv.hard_skills || profile.skills || '');
  const education=cvJson(cv.education);
  const work=cvJson(cv.work_experience);
  const awards=cvJson(cv.awards);
  const certs=cvJson(cv.professional_certifications);
  const qualificationFallback=cvList(profile.qualifications||'');
  const certificationFallback=cvList(profile.certifications||'');
  const eduItems=education.length?education:qualificationFallback.map(x=>({degree:x,field:''}));
  const certItems=certs.length?certs:certificationFallback.map(x=>({title:x,issuer:''}));
  const workItems=work;
  const awardItems=awards;
  const verifiedClass=(ok)=>ok?'verified':'pending';
  const secure=!!opts.secure;
  const publicUrl=opts.publicUrl||'';
  const qrMarkup=publicUrl?`<div class="cv-qr"><div id="cvQr"></div><div class="qr-caption">Scan to view<br>digital CV</div></div>`:'';
  const personal=`<section class="cv-lockbar"><div class="lock-icon">🔒</div><div><h3>Personal Details</h3><p>Full information is hidden for your privacy.<br>Request full information to view.</p></div><a class="cv-request" href="/permissions.html?avx_id=${encodeURIComponent(profile.avx_id)}">Request Full Information <span>›</span></a></section>`;
  const listBlock=(title,icon,items,empty)=>`<section class="cv-card ${items.length?'':'empty'}"><div class="cv-card-head"><div class="cv-head-title"><span class="cv-icon">${icon}</span><h2>${title}</h2></div><span class="cv-status ${items.length?'verified':'pending'}">${items.length?'✓ Verified':'Add in Profile'}</span></div>${items.length?`<div class="cv-items">${items.map(x=>`<div class="cv-item"><span class="cv-check">✓</span><div><b>${esc(x.degree||x.title||x.name||x.company||x.award||x.name||'Record')}</b>${x.field?`<small>${esc(x.field)}</small>`:''}${x.issuer?`<small>${esc(x.issuer)}</small>`:''}${x.role?`<small>${esc(x.role)}</small>`:''}${x.dates?`<small>${esc(x.dates)}</small>`:''}${x.description?`<p>${esc(x.description)}</p>`:''}</div><span class="cv-chevron">›</span></div>`).join('')}</div>`:`<p class="cv-empty">${empty}</p>`}</section>`;
  const skillsBlock=(title,icon,items)=>`<section class="cv-card"><div class="cv-card-head"><div class="cv-head-title"><span class="cv-icon">${icon}</span><h2>${title}</h2></div><span class="cv-status ${items.length?'verified':'pending'}">${items.length?'✓ Verified':'Add in Profile'}</span></div><div class="skill-list">${items.length?items.map(x=>`<span><i>✓</i>${esc(x)}</span>`).join(''):`<p class="cv-empty">Add ${title.toLowerCase()} in your Profile.</p>`}</div></section>`;
  const awardsHtml=listBlock('Awards & Recognition','🏆',awardItems,'Add awards and recognition in your Profile.');
  const workHtml=listBlock('Work Experience','💼',workItems,'Add work experience in your Profile.');
  const certHtml=listBlock('Professional Certifications','▣',certItems,'Add professional certifications in your Profile.');
  const eduHtml=listBlock('Education','🎓',eduItems,'Add education in your Profile.');
  const nysc=cv.nysc?`<section class="cv-card"><div class="cv-card-head"><div class="cv-head-title"><span class="cv-icon">👥</span><h2>NYSC</h2></div><span class="cv-status pending">Pending</span></div><div class="cv-item"><span class="cv-check">✓</span><div><b>${esc(cv.nysc)}</b><small>National Youth Service Corps</small></div><span class="cv-chevron">›</span></div></section>`:'';
  const summary=cv.summary||profile.bio||'Prove Your Potential';
  const photo=profile.id?`<img class="cv-avatar-photo" src="/api/public-profile-photo/${encodeURIComponent(profile.avx_id)}" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'" alt="">`:'';
  const avatar=`<div class="cv-avatar">${photo}<span class="cv-avatar-fallback">A<span>♛</span></span></div>`;
  const email=maskEmail(profile.email||''); const phone=maskPhone(profile.phone||'');
  const verifiedCount=verified.length;
  const status=verifiedCount>0?'VERIFIED':'PROFILE';
  const secureNote=secure?'<div class="secure-note">This secure QR link is valid for 30 minutes and expires after one successful scan.</div>':'';
  const html=`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Digital CV • ${esc(profile.full_name)}</title><style>
  :root{--bg:#03020d;--panel:#08081c;--panel2:#0a0922;--line:#35206b;--purple:#8d24ff;--violet:#b85cff;--text:#f5f2ff;--muted:#aaa2c9;--gold:#e9c86a;--good:#a85cff}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 50% -10%,#17103a 0,#060313 42%,#02020a 100%);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif}.cv-wrap{max-width:1000px;margin:auto;padding:28px 20px 46px}.cv-top{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-bottom:18px}.cv-brand{display:flex;align-items:center;gap:12px}.cv-brand-mark{font-size:43px;font-weight:1000;font-style:italic;color:#a85cff;text-shadow:0 0 24px #8d24ff}.cv-brand-mark span{position:absolute;font-size:15px;color:#f3d56d;margin-left:-29px;margin-top:-27px}.cv-brand-name{font-size:30px;font-weight:900;letter-spacing:.12em}.cv-brand-sub{display:block;font-size:8px;letter-spacing:.18em;color:#8d84a8;margin-top:-3px}.cv-top-right{display:flex;align-items:center;gap:9px;color:#cbbcff;font-size:14px;font-weight:800}.cv-top-right .shield{font-size:26px;color:#9e46ff}.cv-hero{position:relative;overflow:hidden;display:grid;grid-template-columns:160px 1fr 200px;gap:22px;align-items:center;padding:28px;border:1px solid #5121a0;border-radius:22px;background:radial-gradient(circle at 72% 60%,#15103b,transparent 32%),linear-gradient(145deg,#090817,#050411);box-shadow:0 0 45px #5e20ff18}.cv-hero:after{content:'';position:absolute;right:16%;top:-90px;width:2px;height:380px;background:linear-gradient(transparent,#782cff,transparent);transform:rotate(34deg);opacity:.65}.cv-avatar{position:relative;width:150px;height:150px;border-radius:50%;border:3px solid #b04cff;display:grid;place-items:center;background:radial-gradient(circle,#28114f,#090515);box-shadow:0 0 35px #8d24ff55;overflow:hidden}.cv-avatar-photo{width:100%;height:100%;object-fit:cover}.cv-avatar-fallback{font-size:75px;font-weight:1000;font-style:italic;color:#b25aff;text-shadow:0 0 22px #8d24ff;display:grid;place-items:center}.cv-avatar-fallback span{position:absolute;font-size:25px;color:#f2d56b;top:17px;left:59px}.cv-hero h1{font-size:34px;margin:0 0 4px;position:relative;z-index:2}.cv-id{display:inline-flex;align-items:center;gap:5px;padding:5px 9px;border-radius:999px;background:#25104d;border:1px solid #7627d7;color:#d6b9ff;font-size:10px;font-weight:900}.cv-tagline{font-size:18px;color:#b899ff;margin:3px 0 18px}.cv-contact{display:grid;gap:9px;color:#e5e0f4;font-size:13px}.cv-contact span{display:flex;align-items:center;gap:9px}.cv-contact i{color:#a64aff;font-style:normal}.cv-verified{justify-self:center;min-width:155px;padding:14px 22px;border-radius:30px;background:linear-gradient(135deg,#9d2bff,#3e2cff);box-shadow:0 0 35px #8d24ff55;text-align:center;font-weight:1000;letter-spacing:.05em}.cv-qr{justify-self:end;text-align:center;z-index:3}.cv-qr #cvQr{width:145px;height:145px;background:#fff;padding:7px;border-radius:15px;border:3px solid #6c35ff;box-shadow:0 0 24px #682dff66}.cv-qr canvas,.cv-qr img{width:100%!important;height:100%!important}.qr-caption{color:#d4c9e9;font-size:12px;margin-top:8px}.cv-lockbar{margin-top:28px;display:flex;align-items:center;gap:15px;padding:19px 22px;border:1px solid #31205d;border-radius:20px;background:linear-gradient(90deg,#09081c,#0a081e);box-shadow:0 15px 40px #0005}.lock-icon{width:54px;height:54px;border-radius:50%;display:grid;place-items:center;background:#30205e;color:#d0b6ff;font-size:24px}.cv-lockbar h3{margin:0 0 4px;font-size:18px}.cv-lockbar p{margin:0;color:#b0a6c7;line-height:1.5;font-size:13px}.cv-request{margin-left:auto;min-width:250px;text-align:center;padding:13px 18px;border-radius:26px;border:1px solid #a33cff;color:#d79bff;text-decoration:none;font-weight:900}.cv-request span{font-size:23px;vertical-align:-2px;margin-left:10px}.cv-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:16px}.cv-card{border:1px solid #281957;border-radius:19px;padding:20px;background:linear-gradient(145deg,#09091d,#070714);box-shadow:0 15px 40px #0005}.cv-card.wide{grid-column:1/-1}.cv-card-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:14px}.cv-head-title{display:flex;align-items:center;gap:12px}.cv-head-title h2{margin:0;font-size:19px}.cv-icon{width:40px;height:40px;border-radius:13px;display:grid;place-items:center;background:linear-gradient(145deg,#5722b5,#2d155f);color:#c78aff;font-size:20px}.cv-status{padding:6px 10px;border-radius:999px;font-size:9px;font-weight:900}.cv-status.verified{color:#c997ff;background:#251047;border:1px solid #7026cc}.cv-status.pending{color:#f0bd62;background:#2b210e;border:1px solid #76521d}.skill-list{display:grid;grid-template-columns:1fr 1fr;gap:10px 25px}.skill-list span{display:flex;gap:9px;align-items:center;font-size:13px;color:#ddd8ea}.skill-list i,.cv-check{width:20px;height:20px;flex:0 0 20px;border-radius:50%;display:grid;place-items:center;background:#8226ed;color:white;font-style:normal;font-size:11px}.cv-items{display:grid;gap:0}.cv-item{display:flex;align-items:center;gap:12px;padding:13px 0;border-bottom:1px solid #211744}.cv-item:last-child{border-bottom:0}.cv-item>div{min-width:0;flex:1}.cv-item b{display:block;font-size:14px;color:#f1edf8}.cv-item small{display:block;color:#a49bbd;margin-top:4px;font-size:11px}.cv-item p{margin:6px 0 0;color:#aaa2c0;font-size:11px;line-height:1.5}.cv-chevron{color:#a55aff;font-size:25px}.cv-empty{color:#77718e;font-size:12px;margin:0;padding:8px 0}.secure-note{margin-top:14px;padding:11px 14px;border:1px solid #5e461b;background:#1a1409;border-radius:12px;color:#e8bd67;font-size:11px}.cv-footer{display:flex;justify-content:space-between;align-items:center;margin-top:28px;padding:16px 4px;color:#77708f;font-size:10px;letter-spacing:.13em}.cv-footer b{font-size:17px;color:#eee9fa;letter-spacing:.12em}.cv-footer a{color:#a95cff;text-decoration:none}@media(max-width:820px){.cv-hero{grid-template-columns:110px 1fr}.cv-avatar{width:105px;height:105px}.cv-avatar-fallback{font-size:52px}.cv-qr{grid-column:1/-1;justify-self:center;display:flex;align-items:center;gap:12px}.cv-qr #cvQr{width:120px;height:120px}.cv-verified{grid-column:2;justify-self:start}.cv-grid{grid-template-columns:1fr}.cv-card.wide{grid-column:auto}.cv-lockbar{flex-wrap:wrap}.cv-request{width:100%;margin-left:0}}@media(max-width:560px){.cv-wrap{padding:18px 12px 35px}.cv-top-right{font-size:11px}.cv-brand-name{font-size:23px}.cv-hero{grid-template-columns:1fr;text-align:left}.cv-avatar{margin:auto}.cv-verified{grid-column:auto;justify-self:center}.cv-qr{grid-column:auto}.cv-contact{font-size:12px}.skill-list{grid-template-columns:1fr}.cv-footer{flex-direction:column;gap:8px;text-align:center}}
  </style></head><body><main class="cv-wrap"><header class="cv-top"><div class="cv-brand"><div class="cv-brand-mark">A<span>♛</span></div><div><div class="cv-brand-name">AVELYX</div><small class="cv-brand-sub">IDENTITY · CREDENTIALS · OPPORTUNITY</small></div></div><div class="cv-top-right"><span class="shield">♢</span><span>DIGITAL CV<br><small style="font-size:10px">VERIFIED PROFILE</small></span></div></header>
  <section class="cv-hero">${avatar}<div><h1>${esc(profile.full_name)} <span class="cv-id">✓ AVELYX ID</span></h1><div class="cv-tagline">${esc(summary)}</div><div class="cv-contact"><span><i>✉</i>${esc(email)}</span><span><i>⌖</i>${esc(profile.location||'Location not added')}</span><span><i>◉</i>${esc(phone)}</span></div></div><div class="cv-verified">🛡 VERIFIED</div>${qrMarkup}</section>
  ${personal}<div class="cv-grid">${skillsBlock('Soft Skills','👥',soft)}${skillsBlock('Hard Skills','⚙',hard)}${eduHtml}${nysc}${awardsHtml}${workHtml}${certHtml}</div>
  <section class="cv-lockbar" style="margin-top:16px"><div class="lock-icon">🔒</div><div><h3>Unlock Full CV Information</h3><p>Get access to complete details including all dates, descriptions and documents.</p></div><a class="cv-request" href="/permissions.html?avx_id=${encodeURIComponent(profile.avx_id)}">Request Full Information <span>›</span></a></section>
  ${secureNote}<footer class="cv-footer"><div><b>AVELYX</b>　•　REAL PEOPLE　•　VERIFIED CREDENTIALS　•　BETTER OPPORTUNITIES</div><a href="/">www.avelyx-ng.com　◉</a></footer></main>${publicUrl?`<script>(function(){function load(){if(window.QRCode){new QRCode(document.getElementById('cvQr'),{text:${JSON.stringify(publicUrl)},width:130,height:130});return}var s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js';s.onload=load;document.head.appendChild(s)}load()})();</script>`:''}</body></html>`;
  return new Response(html,{status:opts.status||200,headers:{'content-type':'text/html; charset=UTF-8','cache-control':secure?'no-store':'public, max-age=60'}});
}
...
const appPages = {
  ...
  '/profile.html':'profile', '/admin-login.html':'admin-login', '/verification.html':'verification', '/qr.html':'qr', '/permissions.html':'permissions', '/notifications.html':'notifications', '/wallet.html':'wallet', '/opportunities.html':'opportunities', '/verification.html':'verification'
};
...
async function dashboardPage(request, env){
  ...
  <section class="dash-grid"><a class="dash-tile featured" href="/digital-cv.html"><span>01</span><b>Digital CV</b><small>Open your standalone verified professional CV.</small></a><a class="dash-tile" href="/profile.html"><span>02</span><b>My Professional Identity</b><small>Keep your profile complete and ready to share.</small></a><a class="dash-tile" href="/verification.html"><span>03</span><b>Verify Credentials</b><small>Submit supported credentials through the AVELYX verification center.</small></a><a class="dash-tile" href="/wallet.html"><span>04</span><b>AVX Wallet</b><small>View your AVX balance and service-credit activity.</small></a><a class="dash-tile" href="/opportunities.html"><span>05</span><b>Jobs & Opportunities</b><small>Explore professional opportunities matched to your profile.</small></a><a class="dash-tile" href="/permissions.html"><span>06</span><b>Information Permissions</b><small>Control exactly what information you approve for sharing.</small>${pending?`<span class="badge">${pending}</span>`:''}</a><a class="dash-tile" href="/notifications.html"><span>07</span><b>Notifications</b><small>Review important requests and account activity.</small></a></section>
...
async function profilePage(request, env){
  ...
  const digitalCv=await getDigitalCvData(env,user.id);
  const eligibility=await cardEligibility(user,env);
  let credentials=[];
...
<section class="panel-new digital-editor"><div class="section-title"><div><div class="eyebrow gold-text">DIGITAL CV</div><h2>Digital CV Content</h2><p>Everything here is automatically linked to your standalone Digital CV. Save your Profile and the Digital CV updates with it.</p></div><a class="btn ghost" href="/digital-cv.html">View Digital CV</a></div><div class="form-grid" style="margin-top:12px"><div class="full"><label>Digital CV Summary</label><textarea id="cv_summary" ${locked?'disabled':''}>${safe(digitalCv.summary||'')}</textarea></div><div><label>Soft Skills <small>(one per line)</small></label><textarea id="cv_soft_skills" ${locked?'disabled':''}>${safe(digitalCv.soft_skills||'')}</textarea></div><div><label>Hard Skills <small>(one per line)</small></label><textarea id="cv_hard_skills" ${locked?'disabled':''}>${safe(digitalCv.hard_skills||user.skills||'')}</textarea></div><div class="full"><label>Education <small>(one record per line: Degree | Field)</small></label><textarea id="cv_education" ${locked?'disabled':''} placeholder='[{"degree":"Bachelor’s Degree","field":"Computer Science"}]'>${safe(digitalCv.education||'')}</textarea></div><div><label>NYSC</label><input id="cv_nysc" value="${attrSafe(digitalCv.nysc)}" ${locked?'disabled':''}></div><div><label>Awards & Recognition <small>(one record per line: Award | Issuer)</small></label><textarea id="cv_awards" ${locked?'disabled':''} placeholder='[{"award":"Safety Award","issuer":"Daewoo Construction Company"}]'>${safe(digitalCv.awards||'')}</textarea></div><div class="full"><label>Work Experience <small>(one record per line: Company | Role | Dates | Description)</small></label><textarea id="cv_work_experience" ${locked?'disabled':''} placeholder='[{"company":"Company Name","role":"Site Engineer","dates":"2024–2026","description":"Role details"}]'>${safe(digitalCv.work_experience||'')}</textarea></div><div class="full"><label>Professional Certifications <small>(one record per line: Title | Issuer)</small></label><textarea id="cv_professional_certifications" ${locked?'disabled':''} placeholder='[{"title":"Project Management Professional (PMP)","issuer":"Certification Course"}]'>${safe(digitalCv.professional_certifications||'')}</textarea></div></div></section>
...
async function digitalCvPage(request, env){
  const user=await currentUser(request,env);
  if(!user) return Response.redirect(new URL('/login.html?return=/digital-cv.html',request.url),302);
  const shareResponse=await generateShare(request,env);
  const shareData=await shareResponse.json().catch(()=>({}));
  return renderDigitalCv(user,env,{secure:false,publicUrl:shareData.url||new URL('/v/'+user.avx_id,request.url).toString()});
}
function loginPage(){return shell('Login',`<section class="hero"><div class="eyebrow">AVELYX MEMBER ACCESS</div><h1>Welcome back.</h1><p class="muted">Access your profile, secure QR tools and member features.</p></section><section class="panel" style="max-width:560px"><label>Email</label><input id="email" type="email" autocomplete="username" placeholder="you@example.com"><label>Password</label><input id="password" type="password" autocomplete="current-password" placeholder="Your AVELYX password"><div class="actions" style="margin-top:18px"><button class="btn" onclick="login()">Log In</button><a class="btn ghost" href="/register.html">Create Account</a></div><div id="msg"></div></section>`,`<script>async function login(){const msg=document.getElementById('msg');const email=document.getElementById('email').value.trim();const password=document.getElementById('password').value;if(!email||!password){msg.className='error';msg.textContent='Enter your email and password.';return}msg.className='muted';msg.textContent='Signing in...';try{const r=await fetch('/api/login',{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify({email,password})});const d=await r.json().catch(()=>({}));if(!r.ok){msg.className='error';msg.textContent=d.error||d.detail||'Login failed. Please try again.';if(d.needs_verification)msg.innerHTML=(d.error||'Please verify your email.')+' <a href="/verify-email.html?email='+encodeURIComponent(email)+'" style="color:#73bdff">Verify email</a>';return}if(d.requires_2fa){document.querySelector('.panel').innerHTML='<label>Authenticator code</label><input id="code" inputmode="numeric" maxlength="6" placeholder="000000"><div class="actions" style="margin-top:18px"><button class="btn" onclick="finish2fa()">Continue</button></div><div id="msg"></div>';return}location.href='/dashboard.html'}catch(e){msg.className='error';msg.textContent='Login failed. Please try again.'}}async function finish2fa(){const msg=document.getElementById('msg');const r=await fetch('/api/login/2fa',{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify({code:document.getElementById('code').value})});const d=await r.json().catch(()=>({}));if(!r.ok){msg.className='error';msg.textContent=d.error||d.detail||'Invalid authentication code.';return}location.href='/dashboard.html'}</script>`);}
...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname.startsWith('/api/public-profile-photo/') && request.method === 'GET') return await publicProfilePhoto(request, env, decodeURIComponent(url.pathname.slice('/api/public-profile-photo/'.length)));
    ...
    if (url.pathname.startsWith('/s/')) return await scanShare(url.pathname.slice(3), env, request);
    ...
    if (url.pathname.startsWith('/v/')) { const p=await env.DB.prepare('SELECT * FROM profiles WHERE avx_id=?').bind(url.pathname.slice(3)).first(); return p ? await renderDigitalCv(p,env,{secure:false,publicUrl:new URL(url.pathname,request.url).toString()}) : new Response('Profile not found',{status:404}); }
