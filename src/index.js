...
async function sendVerificationEmail(env, email, name, code) {const safeName = esc(name); const html = `<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:30px;background:#071025;color:#fff"><div style="font-size:24px;font-weight:900;letter-spacing:4px">AVELIX</div><h1>Verify your email</h1><p>Hello ${safeName}, use this code to verify your AVELIX account:</p><div style="font-size:34px;font-weight:900;letter-spacing:10px;background:#111b3d;padding:18px;text-align:center;border-radius:14px">${code}</div><p>This code expires in ${VERIFY_MINUTES} minutes. If you did not create this account, you can ignore this email.</p></div>`;
...
async function adminCreditWallet(request, env) {
  ...
  constnewIssued=Number(treasury.issued_amount)+amount;
  await env.DB.batch([
