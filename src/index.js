...
async function scanShare(token, env) {
  ...
  const data = await getDigitalCVByProfileId(profile.id,env);
  return new Response(`<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Digital CV • AVELYX</title>${styles()}${digitalCVStyles()}</head><body>${digitalCVMarkup(data,true)}</body></html>`,{headers:{'content-type':'text/html;charset=UTF-8'}});
}
...
async function getDigitalCVByProfileId(profileId, env) {
  await ensureDigitalCVTable(env);
  const profile = await env.DB.prepare('SELECT * FROM profiles WHERE id=?').bind(profileId).first();
  if (!profile) return null;
  let extra = await env.DB.prepare('SELECT * FROM digital_cv_data WHERE profile_id=?').bind(profileId).first();
  if (!extra) {
    await env.DB.prepare('INSERT OR IGNORE INTO digital_cv_data (profile_id) VALUES (?)').bind(profileId).run();
    extra = await env.DB.prepare('SELECT * FROM digital_cv_data WHERE profile_id=?').bind(profileId).first();
  }
  let credentials=[];
  try {
    const r=await env.DB.prepare("SELECT credential_id,title,credential_type,issuer,reference,status,created_at FROM credentials WHERE profile_id=? AND status='verified' ORDER BY id DESC").bind(profileId).all();
    credentials=r.results||[];
  } catch(e) {}
...
function cvEscList(value) {
  return cvList(value).map(x=>`<span class="cv-pill">${esc(x)}</span>`).join('');
}
function digitalCVMarkup(data, publicMode=false) {
  const p=data.profile||{}, e=data.extra||{};
  const name=p.full_name||'AVELYX Member';
  const title=p.title||'Professional';
  const summary=e.summary||p.bio||'Professional identity and verified potential powered by AVELYX.';
  const skills=cvList(p.skills||e.hard_skills);
  const hard=cvList(e.hard_skills||p.skills);
  const soft=cvList(e.soft_skills);
  const education=cvList(e.education||p.qualifications);
  const certs=cvList(e.professional_certifications||p.certifications);
  const work=cvList(e.work_experience);
  const awards=cvList(e.awards);
  const nysc=e.nysc||'';
  const contact=publicMode?'':`<div class="cv-contact"><span>${esc(p.login_email||p.email||'')}</span>${p.phone?`<span>${esc(p.phone)}</span>`:''}</div>`;
  return `<main class="cv-wrap">
    <section class="cv-head">
      <div><div class="cv-brand">AVELYX</div><div class="cv-eyebrow">VERIFIED DIGITAL CV</div><h1>${esc(name)}</h1><h2>${esc(title)}${p.organization?` <span>•</span> ${esc(p.organization)}`:''}</h2><p class="cv-location">${esc(p.location||'')} ${p.industry?`• ${esc(p.industry)}`:''}</p>${contact}</div>
      <div class="cv-id"><small>AVELYX ID</small><strong>${esc(p.avx_id||'')}</strong><span>● ${esc(String(p.status||'ACTIVE').toUpperCase())}</span></div>
    </section>
    <section class="cv-grid">
      <div class="cv-main">
        <article class="cv-section"><div class="cv-label">PROFILE SUMMARY</div><p>${esc(summary)}</p></article>
        ${work.length?`<article class="cv-section"><div class="cv-label">WORK EXPERIENCE</div>${work.map(x=>`<div class="cv-entry">${esc(x)}</div>`).join('')}</article>`:''}
        ${education.length?`<article class="cv-section"><div class="cv-label">EDUCATION & QUALIFICATIONS</div><div class="cv-pills">${cvEscList(e.education||p.qualifications)}</div></article>`:''}
        ${certs.length?`<article class="cv-section"><div class="cv-label">PROFESSIONAL CERTIFICATIONS</div><div class="cv-pills">${cvEscList(e.professional_certifications||p.certifications)}</div></article>`:''}
        ${data.credentials?.length?`<article class="cv-section"><div class="cv-label">AVELYX VERIFIED CREDENTIALS</div>${data.credentials.map(c=>`<div class="cv-entry"><b>${esc(c.title||c.credential_type||'Credential')}</b><br><span>${esc(c.issuer||'')} ${c.reference?`• ${esc(c.reference)}`:''}</span><em>VERIFIED</em></div>`).join('')}</article>`:''}
      </div>
      <aside class="cv-side">
        <article class="cv-side-card"><div class="cv-label">SKILLS</div><div class="cv-pills">${skills.length?skills.map(x=>`<span class="cv-pill">${esc(x)}</span>`).join(''):'<span class="cv-muted">Add skills in your profile.</span>'}</div></article>
        ${hard.length?`<article class="cv-side-card"><div class="cv-label">HARD SKILLS</div><div class="cv-pills">${cvEscList(e.hard_skills)}</div></article>`:''}
        ${soft.length?`<article class="cv-side-card"><div class="cv-label">SOFT SKILLS</div><div class="cv-pills">${cvEscList(e.soft_skills)}</div></article>`:''}
        ${nysc?`<article class="cv-side-card"><div class="cv-label">NYSC</div><p>${esc(nysc)}</p></article>`:''}
        ${awards.length?`<article class="cv-side-card"><div class="cv-label">AWARDS</div><div class="cv-pills">${cvEscList(e.awards)}</div></article>`:''}
        <article class="cv-side-card"><div class="cv-label">IDENTITY</div><p><b>${esc(p.account_type||'individual').toUpperCase()}</b><br>${esc((p.card_tier||'basic').toUpperCase())} MEMBER</p>${p.website?`<a href="${esc(p.website)}" target="_blank" rel="noopener">${esc(p.website)}</a>`:''}</article>
      </aside>
    </section>
    <footer class="cv-foot">AVELYX — PROVE YOUR POTENTIAL. ${publicMode?'':'<a href="/digital-cv.html">Manage Digital CV</a>'}</footer>
  </main>`;
}
function digitalCVStyles(){return `<style>
.cv-wrap{max-width:1100px;margin:0 auto;padding:26px 18px 60px}.cv-head{display:flex;justify-content:space-between;gap:30px;padding:36px;border:1px solid rgba(139,77,255,.35);border-radius:30px;background:radial-gradient(circle at 85% 15%,rgba(139,77,255,.28),transparent 30%),linear-gradient(135deg,#070a18,#13142c 58%,#090b16);box-shadow:0 25px 80px #0007}.cv-brand{font-weight:1000;letter-spacing:.22em;font-size:22px}.cv-eyebrow,.cv-label{font-size:10px;letter-spacing:.18em;color:#b7a0ff;font-weight:900}.cv-eyebrow{margin-top:28px}.cv-head h1{font-size:clamp(38px,6vw,68px);margin:8px 0 5px;letter-spacing:-2px}.cv-head h2{font-size:20px;margin:0;color:#d9def2}.cv-head h2 span{color:#c6a75b}.cv-location{color:#9faac9}.cv-contact{display:flex;gap:14px;flex-wrap:wrap;color:#bac5e4;font-size:13px}.cv-id{min-width:190px;align-self:flex-start;padding:20px;border-radius:20px;background:#090d1c;border:1px solid #283253}.cv-id small,.cv-id strong,.cv-id span{display:block}.cv-id small{color:#7e8bad;font-size:9px;letter-spacing:.15em}.cv-id strong{margin:8px 0;letter-spacing:.1em}.cv-id span{font-size:10px;color:#68e2a0}.cv-grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(260px,1fr);gap:18px;margin-top:18px}.cv-section,.cv-side-card{background:#0b1022;border:1px solid #202a49;border-radius:22px;padding:24px;margin-bottom:18px}.cv-section p,.cv-side-card p{color:#b6c1df;line-height:1.75}.cv-entry{position:relative;padding:15px 45px 15px 0;border-bottom:1px solid #202a49;color:#dfe5f7;line-height:1.6}.cv-entry:last-child{border-bottom:0}.cv-entry span{color:#9da9c8;font-size:13px}.cv-entry em{position:absolute;right:0;top:16px;color:#6fe3a5;font-size:9px;font-style:normal;font-weight:900}.cv-pills{display:flex;gap:8px;flex-wrap:wrap;margin-top:13px}.cv-pill{display:inline-block;padding:8px 11px;border-radius:999px;background:#121b38;border:1px solid #29375e;color:#bfcaff;font-size:11px}.cv-muted{color:#7886a8;font-size:12px}.cv-side-card a{color:#78c4ff;word-break:break-all;font-size:12px}.cv-foot{text-align:center;color:#697696;font-size:11px;padding-top:4px}.cv-foot a{color:#9e85ff;margin-left:10px}@media(max-width:800px){.cv-head{flex-direction:column}.cv-id{min-width:0}.cv-grid{grid-template-columns:1fr}}
</style>`}
async function digitalCVPage(request,env){
  const user=await currentUser(request,env); if(!user) return new Response('Please log in.',{status:401});
  const data=await getDigitalCVByProfileId(user.id,env);
  return new Response(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Digital CV • AVELYX</title>${styles()}${digitalCVStyles()}</head><body>${digitalCVMarkup(data,false)}<section class=\"cv-edit\" style=\"max-width:1100px;margin:0 auto;padding:0 18px 50px\"><div class=\"panel\"><h2>Digital CV Details</h2><p class=\"muted\">Profile fields update automatically. Use these fields for CV-specific information.</p><div class=\"grid\"><div class=\"wide\"><label>CV Summary</label><textarea id=\"summary\">${esc(data.extra?.summary||'')}</textarea></div><div><label>Hard Skills</label><textarea id=\"hard_skills\">${esc(data.extra?.hard_skills||'')}</textarea></div><div><label>Soft Skills</label><textarea id=\"soft_skills\">${esc(data.extra?.soft_skills||'')}</textarea></div><div><label>Education</label><textarea id=\"education\">${esc(data.extra?.education||'')}</textarea></div><div><label>NYSC</label><textarea id=\"nysc\">${esc(data.extra?.nysc||'')}</textarea></div><div><label>Awards</label><textarea id=\"awards\">${esc(data.extra?.awards||'')}</textarea></div><div class=\"wide\"><label>Work Experience</label><textarea id=\"work_experience\">${esc(data.extra?.work_experience||'')}</textarea></div><div class=\"wide\"><label>Professional Certifications</label><textarea id=\"professional_certifications\">${esc(data.extra?.professional_certifications||'')}</textarea></div></div><div class=\"actions\"><button class=\"btn\" onclick=\"saveDigitalCV()\">Save Digital CV</button><span id=\"cvmsg\"></span></div></div></section><script>
async function saveDigitalCV(){const ids=['summary','soft_skills','hard_skills','education','nysc','awards','work_experience','professional_certifications'];const b={};ids.forEach(id=>b[id]=document.getElementById(id)?.value||'');const r=await fetch('/api/digital-cv',{method:'PUT',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify(b)});const d=await r.json().catch(()=>({}));const m=document.getElementById('cvmsg');m.textContent=r.ok?'Digital CV saved. Reloading…':(d.error||'Unable to save Digital CV.');if(r.ok)setTimeout(()=>location.reload(),400)}
</script></body></html>`,{headers:{'content-type':'text/html;charset=UTF-8'}});
}
async function digitalCVApi(request,env){
  const user=await currentUser(request,env); if(!user) return json({error:'Please log in.'},401);
  const data=await getDigitalCVByProfileId(user.id,env);
  return json({ok:true,profile:data.profile,digital_cv:data.extra,credentials:data.credentials});
}
async function digitalCVSave(request,env){
  const user=await currentUser(request,env); if(!user) return json({error:'Please log in.'},401);
  await ensureDigitalCVTable(env); const b=await request.json().catch(()=>({}));
  const fields=['summary','soft_skills','hard_skills','education','nysc','awards','work_experience','professional_certifications'];
  const values=fields.map(k=>String(b[k]??'').trim().slice(0,10000));
  await env.DB.prepare(`INSERT INTO digital_cv_data (profile_id,summary,soft_skills,hard_skills,education,nysc,awards,work_experience,professional_certifications,updated_at) VALUES (?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)
    ON CONFLICT(profile_id) DO UPDATE SET summary=excluded.summary,soft_skills=excluded.soft_skills,hard_skills=excluded.hard_skills,education=excluded.education,nysc=excluded.nysc,awards=excluded.awards,work_experience=excluded.work_experience,professional_certifications=excluded.professional_certifications,updated_at=CURRENT_TIMESTAMP`).bind(user.id,...values).run();
  return json({ok:true});
}
const appPages = {
  '/register.html': 'register', '/login.html':'login', '/verify-email.html':'verify', '/dashboard.html':'dashboard',
  '/profile.html':'profile', '/digital-cv.html':'digital-cv', '/qr.html':'qr', '/permissions.html':'permissions', '/notifications.html':'notifications', '/wallet.html':'wallet', '/opportunities.html':'opportunities'
};
function shell(title, content, script='') { return new Response(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} • AVELYX</title>${styles()}<style>.app{max-width:1050px;margin:auto;padding:24px 18px 60px}.nav{display:flex;justify-content:space-between;align-items:center;padding:10px 0 34px}.nav a{color:#b8c3e1;text-decoration:none;margin-left:18px}.btn{border:0;border-radius:12px;padding:13px 18px;font-weight:800;cursor:pointer;color:white;background:linear-gradient(135deg,var(--purple),var(--blue));box-shadow:0 10px 30px #315bff22}.btn.ghost{background:#111a36;border:1px solid var(--line);box-shadow:none}.notify-btn{position:absolute;right:4px;top:4px;width:48px;height:48px;border-radius:16px;border:1px solid var(--line);background:#111a36;color:#fff;font-size:21px;cursor:pointer}.notify-badge{position:absolute;right:-4px;top:-5px;min-width:20px;height:20px;padding:0 5px;border-radius:999px;background:#ff477e;color:#fff;font:800 11px/20px system-ui;justify-content:center;align-items:center}.permission-checks{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-top:12px}.check{margin:0;padding:10px;border:1px solid var(--line);border-radius:10px;background:#070c1d}.check input{width:auto;margin-right:8px}.permission-card{margin-bottom:10px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}.panel{background:#0b122a;border:1px solid var(--line);border-radius:22px;padding:24px}.panel h2{margin-top:0}.muted{color:var(--muted)}label{display:block;font-size:12px;color:#9aa8c9;margin:13px 0 7px}input,textarea,select{width:100%;padding:13px 14px;border-radius:12px;border:1px solid #263354;background:#070d20;color:white;outline:none}textarea{min-height:100px;resize:vertical}.wide{grid-column:1/-1}.toast{margin-top:12px;color:#7ee8b1}.error{color:#ff91a5;margin-top:12px}.hero{padding:34px 0 22px}.hero h1{font-size:46px}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.soon{opacity:.62;position:relative;overflow:hidden}.soon:after{content:'COMING SOON';position:absolute;top:15px;right:-31px;transform:rotate(35deg);background:#1d2850;padding:6px 38px;font-size:9px;letter-spacing:.12em}.card-showcase{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:18px}.card-image{width:100%;display:block;border-radius:18px;border:1px solid #29365c;box-shadow:0 20px 50px #0008}.soon-label{display:inline-block;margin:0 0 10px;padding:5px 9px;border-radius:999px;background:#101b3b;color:#a9b9df;font-size:9px;letter-spacing:.14em;font-weight:900}@media(max-width:760px){.card-showcase{grid-template-columns:1fr}}.qrbox{text-align:center}.qrbox canvas,.qrbox img{max-width:260px;margin:16px auto;display:block;background:white;padding:10px;border-radius:14px}.pill{display:inline-block;padding:7px 10px;border-radius:999px;background:#101b3b;color:#9eb1de;font-size:11px}.share-url{word-break:break-all;color:#73bdff;font-size:12px}.actions{display:flex;gap:10px;flex-wrap:wrap}.avx-actions{display:flex;gap:14px;margin-top:18px}.avx-icon{width:52px;height:52px;border-radius:16px;border:1px solid #202a49;background:#070b16;color:#46506d;font-size:24px;font-weight:900;cursor:not-allowed;box-shadow:none}.avx-icon.locked{opacity:.45;filter:grayscale(1)}.avx-icon svg{width:24px;height:24px;display:block;margin:auto}@media(max-width:760px){.grid,.cards{grid-template-columns:1fr}.hero h1{font-size:35px}h1{font-size:34px}}</style></head><body><div class="app"><nav class="nav"><a href="/" style="font-weight:900;font-size:22px;letter-spacing:.15em;color:white">AVELYX</a><div><a href="/dashboard.html">Dashboard</a><a href="/digital-cv.html">Digital CV</a><a href="/login.html">Login</a></div></nav>${content}</div>${script}</body></html>`, {headers:{'content-type':'text/html; charset=UTF-8'}}); }
...
async function profilePage(request, env){
  ...
  const journey=(active)=>[['individual','Basic Individual','01'],['entrepreneur','Platinum Professional','02'],['business','Gold Business','03']].map(([k,n,num])=>`<div class="journey-step ${k===active?'active':''}"><span>${num}</span><div><b>${n}</b><small>${k===active?'CURRENT LEVEL':'Available as you grow'}</small></div></div>`).join('');
...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname.startsWith('/v/')) { const p=await env.DB.prepare('SELECT id FROM profiles WHERE avx_id=?').bind(url.pathname.slice(3)).first(); if(!p)return new Response('Profile not found',{status:404}); const data=await getDigitalCVByProfileId(p.id,env); return new Response(`<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Digital CV • AVELYX</title>${styles()}${digitalCVStyles()}</head><body>${digitalCVMarkup(data,true)}</body></html>`,{headers:{'content-type':'text/html;charset=UTF-8'}}); }
