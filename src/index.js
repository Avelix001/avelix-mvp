...
async function submitCredentialVerification(request, env){
  ...
  const cost=await verificationCost(env);
  const wallet=await env.DB.prepare(`SELECT balance FROM wallets WHERE user_id=?`).bind(user.user_id).first();
  if(!wallet) await env.DB.prepare(`INSERT OR IGNORE INTO wallets(user_id,balance) VALUES(?,0)`).bind(user.user_id).run();
  const fresh=await env.DB.prepare(`SELECT balance FROM wallets WHERE user_id=?`).bind(user.user_id).first();
  if(Number(fresh?.balance||0)<cost) return json({error:`You need ${cost} AVX for this verification. Your current balance is ${Number(fresh?.balance||0)} AVX.`},402);
  const ref=reference||makeVerificationReference();
  const debit=await env.DB.prepare(`UPDATE wallets SET balance=balance-? WHERE user_id=? AND balance>=?`).bind(cost,user.user_id,cost).run();
  ...
  try {
    ...
    await env.DB.prepare('UPDATE wallets SET balance=balance+? WHERE user_id=?').bind(cost,user.user_id).run();
...
function walletPage(){return shell('AVX Wallet',`<section class="hero"><div class="eyebrow">AVX WALLET</div><h1>Your Wallet</h1><p class="muted">AVX is an internal AVELYX service-credit system. Members can use credits for AVELYX services; ordinary peer-to-peer transfer, trading and cash-out are disabled.</p></section><section class="panel"><div class="field"><small class="muted">CURRENT BALANCE</small><div id="balance" style="font-size:42px;font-weight:900;margin-top:8px">0 AVX</div></div><div class="avx-actions"><button class="avx-icon locked" title="Buy AVX" aria-label="Buy AVX unavailable" disabled><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 8h12v10H6z"/><path d="M8 8V6a4 4 0 0 1 8 0v2"/><path d="M9 13h6"/></svg></button><button class="avx-icon locked" title="Use AVX" aria-label="Use AVX unavailable" disabled><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12h16"/><path d="m13 5 7 7-7 7"/></svg></button><button class="avx-icon locked" title="Send unavailable" aria-label="Send unavailable" disabled><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></button><button class="avx-icon locked" title="Cash-out unavailable" aria-label="Cash-out unavailable" disabled><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M8 12h8"/></svg></button></div></section><section class="panel" style="margin-top:16px"><h2>AVX Activity</h2><div id="history">Loading...</div></section>`,`<style>.field{margin-top:8px}.tx{padding:13px;border:1px solid var(--line);border-radius:14px;background:#070c1d;margin:8px 0;display:flex;justify-content:space-between;gap:12px}.tx .amt{font-weight:900}.tx .neg{color:#ff91a5}.tx .pos{color:#7ee8b1}@media(max-width:600px){.tx{display:block}}</style>`+memberGuardScript()+`<script>(async()=>{const p=await me();if(!p)return;balance.textContent=Number(p.avx_balance||0).toLocaleString()+' AVX';const r=await fetch('/api/avx/history');const d=await r.json();history.innerHTML=(d.transactions||[]).map(x=>'<div class="tx"><div><b>'+esc(x.type||'AVX activity')+'</b><br><small class="muted">'+esc(x.reference||'')+' · '+esc(x.created_at||'')+'</small><br><small class="muted">'+esc(x.note||'')+'</small></div><div class="amt '+(Number(x.amount)<0?'neg':'pos')+'">'+(Number(x.amount)>0?'+':'')+Number(x.amount||0).toLocaleString()+' AVX</div></div>').join('')||'<p class="muted">No AVX transactions yet.</p>'})()</script>`)}
