...
function dashboardPage(){return shell('Dashboard',`<section class="hero"><div class="eyebrow">MY AVELYX</div><h1 id="welcome">Welcome</h1><p class="muted">Your professional identity, verification tools and opportunities — organized in one place.</p></section><section class="panel"><div class="eyebrow">QUICK ACCESS</div><h2 style="margin:8px 0 6px">What do you want to manage?</h2><p class="muted">Each area now has its own page so your dashboard stays clean.</p><div class="quick-grid"><a class="quick-card" href="/profile.html"><span class="quick-icon">◉</span><b>My Profile</b><small>Edit your professional information and profile level.</small></a><a class="quick-card" href="/qr.html"><span class="quick-icon">▣</span><b>Secure QR</b><small>Generate a temporary one-scan profile QR.</small></a><a class="quick-card" href="/permissions.html"><span class="quick-icon">✓</span><b>Information Permissions</b><small>Request or approve access to protected information.</small></a><a class="quick-card" href="/notifications.html"><span class="quick-icon">🔔</span><b>Notifications</b><small>See permission requests and important account alerts.</small><span id="notificationBadge" class="badge" style="display:none">0</span></a><a class="quick-card" href="/wallet.html"><span class="quick-icon">◇</span><b>AVX Wallet</b><small>View your AVX balance and wallet status.</small></a><a class="quick-card" href="/opportunities.html"><span class="quick-icon">↗</span><b>Opportunities</b><small>Find opportunities based on your profile evidence.</small></a></div></section><section class="grid" style="margin-top:16px"><div class="panel"><div class="eyebrow">IDENTITY</div><h2 id="levelTitle">Profile Level</h2><div id="identityBox" class="field">Loading...</div><a class="btn" href="/profile.html" style="display:inline-block;margin-top:14px;text-decoration:none">Manage Profile</a></div><div class="panel"><div class="eyebrow">AVX WALLET</div><h2>Wallet</h2><div id="walletBox" class="field">Loading...</div><a class="btn ghost" href="/wallet.html" style="display:inline-block;margin-top:14px;text-decoration:none">Open Wallet</a></div></section><section class="panel" style="margin-top:16px"><div class="eyebrow">VERIFICATION</div><h2>Verification Centre</h2><p class="muted">Credential verification, identity verification and business verification are being prepared for launch.</p><span class="pill">COMING SOON</span></section>`,`<style>.quick-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:18px}.quick-card{position:relative;display:block;padding:20px;border:1px solid var(--line);border-radius:18px;background:#070c1d;color:white;text-decoration:none;transition:.15s}.quick-card:hover{transform:translateY(-2px);border-color:#40517f}.quick-card b{display:block;font-size:17px;margin:10px 0 6px}.quick-card small{display:block;color:var(--muted);line-height:1.5}.quick-icon{font-size:25px}.badge{position:absolute;right:14px;top:14px;min-width:24px;height:24px;border-radius:999px;background:#ff477e;text-align:center;line-height:24px;font-size:11px;font-weight:900}@media(max-width:760px){.quick-grid{grid-template-columns:1fr 1fr}}@media(max-width:520px){.quick-grid{grid-template-columns:1fr}}</style>`+memberGuardScript()+`<script>(async()=>{const p=await me();if(!p)return;welcome.textContent='Welcome, '+p.full_name;identityBox.innerHTML='<b>'+esc(p.avx_id)+'</b><br><span class="pill">'+esc((p.account_type||'individual').toUpperCase())+'</span> · '+esc((p.card_tier||'basic').toUpperCase())+'<br><small class="muted">'+esc(p.title||'Professional profile')+'</small>';walletBox.innerHTML='<b style="font-size:24px">'+Number(p.avx_balance||0).toLocaleString()+' AVX</b><br><small class="muted">Wallet available · AVX use currently locked</small>';try{const r=await fetch('/api/information-requests',{credentials:'same-origin'});const d=await r.json();const n=(d.requests||[]).filter(x=>x.status==='pending').length;if(n){notificationBadge.style.display='block';notificationBadge.textContent=n}}catch(e){}})()</script>`)}
function profilePage(){return shell('My Profile',`<section class="profile-hero"><div class="profile-glow"></div><div class="verify-orbit" aria-label="AVELYX Verified"><div class="orbit-ring"></div><div class="verify-core"><span class="verify-a">A</span><span class="verify-check">✓</span></div><div class="orbit-label">AVELYX VERIFIED</div></div><div class="profile-hero-copy"><div class="eyebrow">MY PROFILE</div><h1>Professional Identity</h1><p>Build a profile people can trust. Your AVELYX ID stays with you as your professional identity grows.</p><div class="profile-mini-badges"><span>✦ Secure Identity</span><span>◉ Shareable QR</span><span>✓ Permission Controlled</span></div></div></section><section class="profile-layout"><div class="panel profile-panel"><div class="section-head"><div><div class="eyebrow">IDENTITY DETAILS</div><h2>Profile Information</h2><p class="muted">Keep your professional information accurate and ready to share.</p></div><div class="live-dot"><i></i> LIVE</div></div><div id="profileForm"><div class="loading-card"><span class="spinner"></span> Loading your profile…</div></div></div><div class="panel level-panel"><div class="eyebrow">AVELYX STATUS</div><h2>Profile Level</h2><div id="levelBox"><div class="loading-card"><span class="spinner"></span> Loading your level…</div></div><div id="upgradeBox" style="margin-top:16px"></div><div class="level-road"><div class="road-title">YOUR JOURNEY</div><div class="road-step active"><b>01</b><span>Basic Individual</span></div><div class="road-step"><b>02</b><span>Platinum Professional</span></div><div class="road-step"><b>03</b><span>Gold Business</span></div></div></div></section>`,`<style>.profile-hero{position:relative;overflow:hidden;display:flex;align-items:center;gap:30px;padding:34px;border-radius:26px;border:1px solid rgba(100,120,220,.35);background:radial-gradient(circle at 82% 35%,rgba(103,80,255,.35),transparent 30%),linear-gradient(135deg,#080d22,#111a3b 55%,#0b1028);box-shadow:0 22px 60px rgba(0,0,0,.25);min-height:260px}.profile-glow{position:absolute;width:240px;height:240px;border-radius:50%;right:-80px;top:-80px;background:rgba(110,92,255,.18);filter:blur(10px)}.profile-hero-copy{position:relative;z-index:2;max-width:680px}.profile-hero h1{margin:6px 0 10px;font-size:clamp(32px,5vw,52px);letter-spacing:-1.5px}.profile-hero p{margin:0;color:#b9c4e8;line-height:1.65;font-size:15px}.profile-mini-badges{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}.profile-mini-badges span{padding:8px 11px;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:rgba(255,255,255,.05);font-size:11px;font-weight:800;color:#dce4ff}.verify-orbit{position:relative;width:170px;height:170px;flex:0 0 170px;display:grid;place-items:center}.orbit-ring{position:absolute;inset:4px;border-radius:50%;border:1px dashed rgba(152,165,255,.7);animation:spin 13s linear infinite}.orbit-ring:before,.orbit-ring:after{content:'';position:absolute;width:9px;height:9px;border-radius:50%;background:#8b7cff;box-shadow:0 0 18px #8b7cff}.orbit-ring:before{top:10px;left:50%}.orbit-ring:after{bottom:18px;right:10px}.verify-core{width:112px;height:112px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#fff,#cbd4ff);color:#10152e;box-shadow:0 0 0 8px rgba(255,255,255,.06),0 0 42px rgba(123,103,255,.5);position:relative;animation:pulse 3s ease-in-out infinite}.verify-a{font-size:54px;font-weight:1000;font-style:italic;line-height:1}.verify-check{position:absolute;right:8px;bottom:7px;width:31px;height:31px;border-radius:50%;display:grid;place-items:center;background:#151d48;color:white;font-size:19px;font-weight:1000;border:3px solid #fff}.orbit-label{position:absolute;bottom:-4px;white-space:nowrap;font-size:9px;font-weight:1000;letter-spacing:2px;color:#dce3ff;background:#111936;padding:7px 10px;border-radius:999px;border:1px solid rgba(255,255,255,.14)}.profile-layout{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(280px,.8fr);gap:16px;margin-top:18px;align-items:start}.profile-panel,.level-panel{border-color:rgba(91,110,190,.28);box-shadow:0 12px 35px rgba(0,0,0,.12)}.section-head{display:flex;justify-content:space-between;gap:15px;align-items:flex-start}.section-head h2{margin:3px 0}.live-dot{font-size:10px;font-weight:900;letter-spacing:1px;color:#b9c6ff;padding:7px 10px;border-radius:999px;background:rgba(88,105,200,.12)}.live-dot i{display:inline-block;width:7px;height:7px;border-radius:50%;background:#72e6a4;margin-right:5px;box-shadow:0 0 10px #72e6a4}.loading-card{display:flex;align-items:center;gap:10px;padding:18px;border:1px dashed rgba(125,140,205,.3);border-radius:15px;color:#aeb9dc;background:rgba(255,255,255,.025)}.spinner{width:16px;height:16px;border-radius:50%;border:2px solid rgba(255,255,255,.2);border-top-color:#9d91ff;animation:spin .8s linear infinite}.level-panel{background:linear-gradient(160deg,#0b112b,#10183a)}.level-road{margin-top:22px;padding-top:18px;border-top:1px solid rgba(255,255,255,.08)}.road-title{font-size:10px;font-weight:900;letter-spacing:1.5px;color:#8491ba;margin-bottom:12px}.road-step{display:flex;align-items:center;gap:10px;padding:9px 0;color:#7f8caf;font-size:12px}.road-step b{width:27px;height:27px;border-radius:50%;display:grid;place-items:center;border:1px solid rgba(255,255,255,.12);font-size:9px}.road-step.active{color:#e5e9ff}.road-step.active b{background:#7264ff;color:white;border-color:#7264ff;box-shadow:0 0 18px rgba(114,100,255,.4)}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.035)}}@media(max-width:800px){.profile-hero{padding:25px;flex-direction:column;align-items:flex-start}.verify-orbit{width:140px;height:140px;flex-basis:140px}.verify-core{width:92px;height:92px}.verify-a{font-size:44px}.profile-layout{grid-template-columns:1fr}}@media(prefers-reduced-motion:reduce){.orbit-ring,.verify-core{animation:none}}</style>`+memberGuardScript()+`<script>let profile;const labels={individual:'Basic — Individual',entrepreneur:'Platinum — Entrepreneur / Professional',business:'Gold — Business / Organization'};function formHtml(p){return '<div class="field"><b>'+esc(p.avx_id)+'</b><br><small class="muted">Your permanent AVELYX ID</small></div><label>Full name *</label><input id="full_name" value="'+attr(p.full_name)+'"><label>Professional title</label><input id="title" value="'+attr(p.title)+'"><label>Organization / business</label><input id="organization" value="'+attr(p.organization)+'"><label>Industry</label><input id="industry" value="'+attr(p.industry)+'"><label>Location</label><input id="location" value="'+attr(p.location)+'"><label>Phone</label><input id="phone" value="'+attr(p.phone)+'"><label>Website</label><input id="website" value="'+attr(p.website)+'"><label>CAC / RC number</label><input id="cac_number" value="'+attr(p.cac_number)+'"><label>Skills</label><input id="skills" value="'+attr(p.skills)+'" placeholder="Python, CAD, marketing..."><label>Qualifications</label><textarea id="qualifications">'+esc(p.qualifications)+'</textarea><label>Certifications</label><textarea id="certifications">'+esc(p.certifications)+'</textarea><label>About</label><textarea id="bio">'+esc(p.bio)+'</textarea><button class="btn" style="margin-top:14px" onclick="saveProfile()">Save Changes</button><div id="msg"></div>'}function renderLevel(){const levelBox=document.getElementById('levelBox'),upgradeBox=document.getElementById('upgradeBox');const t=profile.account_type||'individual';levelBox.innerHTML='<span class="pill">'+labels[t]+'</span><div class="field" style="margin-top:14px"><b>AVELYX ID</b><br>'+esc(profile.avx_id)+'<br><b style="display:block;margin-top:10px">Card Tier</b>'+esc((profile.card_tier||'basic').toUpperCase())+'</div>';if(t==='individual')upgradeBox.innerHTML='<p class="muted">Grow your profile when you are ready.</p><button class="btn" onclick="upgrade(\'entrepreneur\')">Upgrade to Platinum</button>';else if(t==='entrepreneur')upgradeBox.innerHTML='<span class="pill">PLATINUM ACTIVE</span><p class="muted">Ready to establish a business identity?</p><button class="btn" onclick="upgrade(\'business\')">Upgrade to Gold Business</button>';else upgradeBox.innerHTML='<span class="pill">GOLD BUSINESS ACTIVE</span><p class="muted">Your business-level identity is active.</p>'}async function saveProfile(){const r=await fetch('/api/profile',{method:'PUT',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify({full_name:document.getElementById('full_name').value,email:profile.login_email||profile.email,title:document.getElementById('title').value,organization:document.getElementById('organization').value,industry:document.getElementById('industry').value,location:document.getElementById('location').value,phone:document.getElementById('phone').value,website:document.getElementById('website').value,cac_number:document.getElementById('cac_number').value,skills:document.getElementById('skills').value,qualifications:document.getElementById('qualifications').value,certifications:document.getElementById('certifications').value,bio:document.getElementById('bio').value})});const d=await r.json();const msg=document.getElementById('msg');msg.className=r.ok?'toast':'error';msg.textContent=r.ok?'Profile saved successfully.':(d.error||'Unable to save profile.');if(r.ok){profile=(await me());renderLevel()}}async function upgrade(target){let body={target_type:target};if(target==='business'){body.organization=prompt('Business / organization name:',profile.organization||'');if(!body.organization)return;body.industry=prompt('Industry:',profile.industry||'')||profile.industry;body.cac_number=prompt('CAC / RC number (optional):',profile.cac_number||'')||profile.cac_number}const r=await fetch('/api/profile/upgrade',{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify(body)});const d=await r.json();if(!r.ok){alert(d.error||'Unable to upgrade');return}alert(d.message);profile=await me();renderLevel()}(async()=>{try{profile=await me();if(!profile)return;document.getElementById('profileForm').innerHTML=formHtml(profile);renderLevel()}catch(e){document.getElementById('profileForm').innerHTML='<div class="error">Unable to load profile. Please refresh and try again.</div>';document.getElementById('levelBox').innerHTML='<div class="error">Unable to load profile level.</div>';console.error(e)}})()</script>`)}
